...
public class OefullinfoServiceImpl extends ServiceImpl<OefullinfoMapper, Oefullinfo> implements IOefullinfoService {
    ...
    public List<Map<String, Object>> selOefullList(Page<Map<String, Object>> page, String fctime, String vehicleid, String caseno, Integer prostatus) {
        ...
        return oefullinfoMapper.selOefullList(page, fctime, start, end, vehicleid, caseno, prostatus);
    }
    ...
    @Override
    public List<Map<String, Object>> getAllLawdocTypeList() {
        return oefullinfoMapper.getAllLawdocTypeList();
    }

    @Override
    public ResponseEntity<FileSystemResource> createAndDownloadLawdoc(Integer oeid, Integer[] ldtypes, HttpServletRequest req) throws Exception {
        ...
        //jar包安装路径
        String jarInstallPath = YUtil.getJarInstallPath().getAbsoluteFile().toString();
        int i = jarInstallPath.indexOf(":");
        String driverName = jarInstallPath.substring(i - 1, i + 1);
        ...
        for (Integer ldtype : lawdocTypes) {
            //将返回的文书地址存入list
            String url = createOefullLawdoc(oeid, String.valueOf(ldtype));
            lawdocurlList.add(driverName + url);
        }

        /**
         * 文书目录的路径
         */
        String directory = createDirectory(oeid,ldtypes);
        lawdocurlList.add(1,driverName + directory);
        ...
        if (lawdocurlList.size() > 0) {
            ...
            Document document = wordAdd.AddDocs2DocByContinuous(lawdocurlList, driverName + outPath);
        }
        return YUtil.export(new File(outPath), "文书.docx");
    }
    ...
    @Override
    public Oefullinfo findOefullInfoByOseId(Integer id, Integer prostatus) {
        return oefullinfoMapper.findOefullInfoByOseId(id,prostatus);
    }
    ...
    @Override
    public Oefullinfo findOefullInfoByLscId(Integer id,Integer prostatus) {
        return oefullinfoMapper.findOefullInfoByLscId(id,prostatus);
    }

    @Override
    public Oefullinfo findById(Integer oefullinfoId) {
        return oefullinfoMapper.findById(oefullinfoId);
    }
	 ...
	 */
	@Override
	public Map<String,Object> findOefullInfoBrowse(Integer id, Integer prostatus, String casetype) {
		
		return oefullinfoMapper.findOefullInfoBrowse(id, prostatus, casetype);
	}
}
