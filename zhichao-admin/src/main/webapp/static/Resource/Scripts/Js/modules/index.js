var $,tab;
layui.config({
	base : "Resource/Scripts/Js/modules/"
	...
	$('.lockcms').mouseover(function(){
	   	   layer.tips('请按Alt+L快速锁屏！', '.lockcms', {
	             tips: [1, '#FF5722'],
	             time: 4000
	       });
	   })
	...
	$("body").on("click","#unlock",function(){
		if($(this).siblings(".admin-header-lock-input").val() == ''){
			...
			var unlockpwd = $(this).siblings(".admin-header-lock-input");
			$.ajax({
				url:'/unlock',
				type:'GET',
				dataType:'json',
	...
	// 添加新窗口
	$("body").on("click",".layui-nav .layui-nav-item a:not('.noAddTab'),.top_menu.layui-nav .layui-nav-child a:not('.noAddTab'),.mobileAddTab",function(){
		//如果不存在子级
...
// 2017-12-15 yModel弹窗方法	
function yModel(code, content, options, end, yes, cancel) {
	if (code===1) {
...
function yModelClose(d) {
	layer.close(d);
}
function yModelAllClose(type){
	layer.closeAll(type);
}
