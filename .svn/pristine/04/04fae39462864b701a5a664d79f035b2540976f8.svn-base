...
public class CorpinfoServiceImpl extends ServiceImpl<CorpinfoMapper, Corpinfo> implements ICorpinfoService {
	...
	public List<Corpinfo> selList(Page<Corpinfo> page, String areacode, String vehicleid, String corptime) {
		...
		//开始查询
		List<Corpinfo> corpinfos = mapper.selList(page, code, vehicleid,corptime, begin, end);
		
		//重组汽车类型
		corpinfos.forEach(corpinfo -> corpinfo.setVehicletype(DictUtil.selectNameByEnameNum("cartype", corpinfo.getVehicletype())));
	...
	public Corpinfo selCorpInfo(Integer id) {
		...
		//重组货车类型
		corpinfo.setVehicletype(DictUtil.selectNameByEnameNum("trucksType", corpinfo.getVehicletype()));
